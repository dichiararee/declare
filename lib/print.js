...
import { formatNum } from './numberfix.js' // Importa la funzione globale
...
export default async function (m, conn = {}, isEvent = false) {
  try {
    ...
    console.log(`${border} ${chalk.grey('👤 DA :')} ${chalk.bold(m.pushName || userRaw)} (${chalk.white(userFormatted)}) | ${formatRole(m.isOwner, m.isAdmin)}`)
